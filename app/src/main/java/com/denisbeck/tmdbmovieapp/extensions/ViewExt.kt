...
fun ImageView.insertImageOriginal(posterId: String?) {
    Glide.with(context).load("https://image.tmdb.org/t/p/original$posterId").into(this)
}

fun ChipGroup.addChips(data: List<Genre>?, layoutInflater: LayoutInflater, isCheckable: Boolean = true) {
    data?.let {
        it.forEach {
